...
import FlowerGrowthAnimation from './FlowerGrowthAnimation';
...
interface GiftItem {
  ...
  cardsNeeded?: number;
  emoji: string;
  type?: 'special' | 'theme' | 'companion';
  themeId?: 'original' | 'sunflower' | 'rose';
  companionId?: 'bee' | 'butterfly' | 'cat';
}
...
const COMPANION_GIFTS: GiftItem[] = [
  {
    id: 'companion-bee',
    name: '小蜜蜂',
    desc: '⭐ 10 分 或 🃏 1 張圖鑑 (上限 5 隻)',
    cost: 10,
    cardsNeeded: 1,
    emoji: '🐝',
    type: 'companion',
...
export default function GardenView({
  ...
  // Screen mode states: 'main' | 'exchange' | 'success' | 'management'
  const [mode, setMode] = useState<'main' | 'exchange' | 'success' | 'management'>(initialMode);
  ...
  const handleExchange = (gift: GiftItem, method: 'points' | 'cards' = 'points') => {
    ...
    if (gift.type === 'companion') {
      const comps = plantState.companions || {};
      if (gift.companionId === 'bee') {
        const currentOwned = comps.beeOwned ?? comps.bee ?? 0;
        if (currentOwned >= 5) {
          setToastMessage('🐝 已達到小蜜蜂數量上限（5 隻）');
          setTimeout(() => setToastMessage(null), 3000);
          return;
        }
        const newOwned = Math.min(5, currentOwned + 1);
        const newDisplay = Math.min(5, (comps.beeDisplay ?? comps.bee ?? 0) + 1);
        onUpdatePlantState({
          ...plantState,
          companions: {
            ...comps,
            beeOwned: newOwned,
            beeDisplay: newDisplay,
            bee: newDisplay
          }
        });
      } else if (gift.companionId === 'butterfly') {
        const currentOwned = comps.butterflyOwned ?? comps.butterfly ?? 0;
        if (currentOwned >= 3) {
          setToastMessage('🦋 已達到蝴蝶數量上限（3 隻）');
          setTimeout(() => setToastMessage(null), 3000);
          return;
        }
        const newOwned = Math.min(3, currentOwned + 1);
        const newDisplay = Math.min(3, (comps.butterflyDisplay ?? comps.butterfly ?? 0) + 1);
        onUpdatePlantState({
          ...plantState,
          companions: {
            ...comps,
            butterflyOwned: newOwned,
            butterflyDisplay: newDisplay,
            butterfly: newDisplay
          }
        });
      } else if (gift.companionId === 'cat') {
        const currentOwned = comps.catOwned ?? comps.cat ?? 0;
        if (currentOwned >= 1) {
          setToastMessage('🐱 已擁有小貓咪（上限 1 隻）');
          setTimeout(() => setToastMessage(null), 3000);
          return;
        }
        onUpdatePlantState({
  ...
  const handleUpdateCompanionDisplay = (type: 'bee' | 'butterfly' | 'cat', delta: number) => {
    playClickSound(delta > 0 ? 600 : 450, 'sine');
    const comps = plantState.companions || {};
    if (type === 'bee') {
      const owned = comps.beeOwned ?? comps.bee ?? 0;
      const current = comps.beeDisplay ?? comps.bee ?? 0;
      ...
      const owned = comps.butterflyOwned ?? comps.butterfly ?? 0;
      const current = comps.butterflyDisplay ?? comps.butterfly ?? 0;
      const next = Math.max(0, Math.min(owned, current + delta));
      onUpdatePlantState({
        ...plantState,
        companions: {
          ...comps,
          butterflyDisplay: next,
          butterfly: next
        }
      });
    } else if (type === 'cat') {
      const owned = comps.catOwned ?? comps.cat ?? 0;
      const current = comps.catDisplay ?? comps.cat ?? 0;
      const next = Math.max(0, Math.min(owned, current + delta));
      onUpdatePlantState({
        ...plantState,
        companions: {
  ...
  const handleContinueCaring = () => {
    playSuccessChime();
    setToastMessage('🌸 謝謝你悉心照顧！你對小綠的愛心讓它在花園裡美麗常開。你可以隨時點擊「花園商店」兌換獎勵喔！');
    setTimeout(() => {
      setToastMessage(null);
    }, 4000);
  };
              ...
              <div className="flex-1 flex items-center justify-center relative mt-1 select-none">
                <div className="bg-white p-2 pb-4 rounded shadow-[0_4px_12px_rgba(0,0,0,0.1)] border border-slate-100 w-28 sm:w-32 rotate-[-1.5deg] transform hover:rotate-0 hover:scale-105 transition-all duration-300">
                  <div className="aspect-square w-full bg-linear-to-b from-slate-50 to-slate-100 border border-slate-200/50 overflow-hidden relative flex items-center justify-center rounded-xs">
                    ...
                    <div className="scale-[0.45] sm:scale-[0.48] origin-center w-40 h-40 flex items-center justify-center select-none pointer-events-none">
                      <AnimatedPlant
  ...
  // Render Page: COMPANION DISPLAY MANAGEMENT SCREEN
  if (mode === 'management') {
    const comps = plantState.companions || {};
    const beeOwned = comps.beeOwned ?? comps.bee ?? 0;
    const beeDisplay = comps.beeDisplay ?? comps.bee ?? 0;
    const butterflyOwned = comps.butterflyOwned ?? comps.butterfly ?? 0;
    const butterflyDisplay = comps.butterflyDisplay ?? comps.butterfly ?? 0;
    const catOwned = comps.catOwned ?? comps.cat ?? 0;
    const catDisplay = comps.catDisplay ?? comps.cat ?? 0;

    return (
      <div className="flex-1 flex flex-col justify-between py-2 px-1 h-full overflow-y-auto">
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <button
              ...
              className="flex items-center gap-1 text-[13px] px-3 py-1.5 rounded-xl bg-brand-sand/40 text-brand-moss hover:bg-brand-sand/70 transition font-black cursor-pointer border-0"
            ...
            <h2 className="text-[17px] font-black text-gray-800 font-sans tracking-tight pr-2">
              花園小夥伴管理
            </h2>
            <div className="w-10" />
          </div>
          ...
          {/* List of 3 Companions */}
          <div className="space-y-3 pt-1">
            {/* 🐝 Bee */}
            <div className="bg-white p-4 rounded-2xl border border-brand-sand shadow-xs flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2.5 bg-amber-50/60 p-1.5 rounded-full border border-amber-200/80">
                <button
                  type="button"
                  disabled={beeDisplay <= 0}
                  onClick={() => handleUpdateCompanionDisplay('bee', -1)}
                  className={`w-8 h-8 rounded-full font-black text-base flex items-center justify-center transition border ${
                    beeDisplay > 0
                ...
                <span className="w-5 text-center font-mono font-black text-sm text-gray-800">
                  {beeDisplay}
                </span>
                <button
                  type="button"
      ...
      <div className="flex-1 flex flex-col justify-between py-1 px-1 h-full overflow-y-auto">
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <h2 className="text-[17px] font-black text-gray-800 font-sans tracking-tight pr-4">花園商店</h2>
            <div className="w-10" />
          </div>
          ...
          <div className="space-y-3.5 overflow-y-auto max-h-[460px] pr-0.5 pb-8">
            {GIFTS.map((gift) => {
              const todayStr = new Date().toISOString().split('T')[0];
              const hasCheckedInToday = records.some(r => r.date === todayStr);
            ...
            <div className="text-[12px] font-black text-gray-400 tracking-widest uppercase text-center pt-2 pb-1">
              ─── 花種解鎖 ───
            </div>

            {THEME_GIFTS.map((gift) => {
              const pointsMet = score >= gift.cost;
              const isAffordable = gift.cost === 0 || pointsMet;
              return (
                <div 
                  key={gift.id}
                  className="bg-white p-4 rounded-2xl border border-brand-sand flex items-center justify-between gap-4 shadow-xs hover:border-brand-sage/40 transition-all duration-300"
            ...
            <div className="relative flex items-center justify-center pt-3 pb-1">
              <span className="text-[12px] font-black text-gray-400 tracking-widest uppercase text-center">
                ─── 花園小夥伴 ───
              </span>
              <button
                type="button"
                ...
                className="absolute right-0 bg-amber-100/90 text-amber-900 border border-amber-300 px-3 py-1 rounded-full text-xs font-black flex items-center gap-1 hover:bg-amber-200 transition active:scale-95 cursor-pointer shadow-xs border-0"
              >
                <span>🐾 小夥伴管理</span>
              </button>
            </div>
            ...
            {COMPANION_GIFTS.map((gift) => {
              ...
              let isOwnedLimit = false;
              let ownedCount = 0;
              let maxLimit = 0;

              if (gift.companionId === 'bee') {
                ownedCount = comps.beeOwned ?? comps.bee ?? 0;
                maxLimit = 5;
                isOwnedLimit = ownedCount >= maxLimit;
              } else if (gift.companionId === 'butterfly') {
                ownedCount = comps.butterflyOwned ?? comps.butterfly ?? 0;
                maxLimit = 3;
                isOwnedLimit = ownedCount >= maxLimit;
              } else if (gift.companionId === 'cat') {
                ownedCount = comps.catOwned ?? comps.cat ?? 0;
                maxLimit = 1;
                isOwnedLimit = ownedCount >= maxLimit;
              }

              const isAffordable = !isOwnedLimit && (pointsMet || cardsMet);

              let buttonText = '兌換';
              if (isOwnedLimit) {
                buttonText = '已擁有';
              } else if (gift.companionId === 'cat') {
                buttonText = cardsMet ? '解鎖領取' : '未解鎖';
              }
                ...
                >
                  <div className="space-y-1 text-left flex-1">
                    <h3 className="text-sm font-black text-gray-800 flex items-center gap-1">
                      ...
                      <span>{gift.name}</span>
                      {ownedCount > 0 && (
                        <span className="text-[11px] font-extrabold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200 ml-1">
                          已擁有 {ownedCount} 隻
                        </span>
                      )}
                    </h3>
                    ...
                    {gift.companionId === 'cat' && (
                      <p className="text-xs font-black text-amber-600 flex items-center gap-1 pt-0.5">
                        <span>🃏 圖鑑解鎖進度：{unlockedCardsCount}/10 張</span>
                      </p>
                    )}
                  </div>
                  <div className="shrink-0 flex items-center justify-center">
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-xs font-black transition cursor-pointer text-center min-w-[70px] ${
                        isAffordable
                          ? 'bg-brand-sage text-white hover:bg-brand-moss active:scale-95 shadow-xs border-0'
                          : isOwnedLimit
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <h4 className="text-[16px] font-black text-gray-800">
                        {giftToConfirm.type === 'companion' && (plantState.companions?.[giftToConfirm.companionId!] || 0) >= 2 ? '確定要再增加一隻嗎？' : '確定要兌換嗎？'}
                      </h4>
  ...
  return (
    <div className="flex-1 flex flex-col justify-between py-1 px-1 space-y-3.5 h-full overflow-y-auto relative">
      {/* Centered Garden Title & Growth Showcase Button */}
      <div className="text-center relative shrink-0 space-y-1">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-gradient-to-r from-amber-100 via-orange-100 to-amber-100 hover:from-amber-200 hover:to-orange-200 text-amber-900 border border-amber-300/80 rounded-full text-xs font-black shadow-2xs transition active:scale-95 cursor-pointer"
          title="點擊觀賞向日葵與玫瑰花的 6 階段生長動畫"
        >
          <Sparkles className="w-3.5 h-3.5 text-amber-600 animate-pulse" />
          <span>🌻 觀賞 6 階段花卉生長動畫賞</span>
        </button>
      </div>
      ...
      <div className="text-center space-y-2 shrink-0 px-2">
        ...
        <div className="space-y-1.5 max-w-sm mx-auto w-full pt-1">
          <div className="flex items-center justify-between text-xs sm:text-sm font-black text-brand-moss font-sans px-0.5">
            <span>進度條 （{displayCount}/10） · {stageInfo.stageName}</span>
      ...
      <div className="space-y-3 shrink-0">
        ...
        {checkInCount >= 10 ? (
          ...
          <div className="flex justify-center">
            <button
              ...
              style={{ minHeight: '38px' }}
            >
              <span>🧺 收割花園（未達到收割條件 {displayCount}/10）</span>
            </button>
          </div>
        )}
        ...
        <div className="flex justify-center pt-1">
          <button
            ...
            className="px-8 py-2.5 bg-brand-sage hover:bg-brand-moss text-white rounded-full text-sm font-black shadow-[0_4px_12px_rgba(109,160,111,0.2)] flex items-center justify-center gap-1.5 transition active:scale-95 cursor-pointer border-0 max-w-[260px] w-auto mx-auto"
            style={{ minHeight: '40px' }}
          >
            <span>🎁 前往「花園商店」</span>
          </button>
      ...
      {/* FLOWER GROWTH ANIMATION SHOWCASE MODAL */}
      <AnimatePresence>
        {showGrowthModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
            ...
            <motion.div
              ...
              className="bg-white rounded-3xl border-2 border-brand-sand p-2 sm:p-4 max-w-xl w-full h-[90vh] max-h-[680px] relative z-10 shadow-2xl flex flex-col overflow-hidden"
            >
              <FlowerGrowthAnimation
